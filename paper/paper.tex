...
\begin{abstract}
  ...
  % \SH{Add RL stuff}
  % Therefore, we present a flow time estimation procedure to set lead times dynamically
  % by using an artificial neural network. Additionally, we implement a safety lead time
  % to incorporate the underlying cost ratio in the order release model. We test our
  % proposed approach using a simulation model of a three-stage make-to-order flow-
  % shop and compare the forecast accuracy and the accumulated costs to other forecast-
  % based order release models from literature. We show that our proposed model using
...
\(R_{t}^{\pol}(s) = E_{\pol}[ r(s_{t},a_{t}) \mid s_{t} = s, a_{t} = a]\) % \MS[t]{check if correct!}
...
\begin{figure}[t!]
  ...
  \end{tikzpicture}
  \caption{\label{fig:printer} A MDP with two different determinstic policies. The only action choice is in
...
\begin{algorithm}[t!]
  \begin{algorithmic}[1]
    \State{}Initialize values to \(0\) \While{the stopping criterion is not fulfilled}
    \State{}\begin{minipage}[t]{0.9\textwidth} With probability \(p_{exp}\) choose a random action
      ...
      \frac{1}{1-\gamma_{0}})\)
      % \(\Delta e^\pol_\gamma_{1},\gamma_{0}(s,a) = V^\pol_{\gamma_0}(s,a) - V^\pol_{\gamma_1}(s,a)\)
      for all actions for which
      % \(\frac{\avgrew^{\pol}(s,a)}{1-\gamma_{0}} + \V^{\pol}(s,a) < V^\pol_{\gamma_0}(s,a)\)
...
%   Optimisations:
%   \begin{enumerate}
%   \item Use Lemma 3 of Miller \& Veinott to get rid of W2
%   \item Use Reference state as in Mahadevan to get rid of W
%   \item Average reward freed reinforcement learning
%   \end{enumerate}
% }
