...
\begin{abstract}
  ...
  A major task in hierarchical structured manufacturing planning and control is to determine when to
  release orders to the shop floor. One key parameter is the lead time which is the planned time
...
Reinforcement learning is an optimisation technique that stems from dynamic programming, therefore
also seeks to compute the best stationary policy for a given problem. The idea is to evaluate the
current system state according to the direct feedback (reward), as well as the previously learned
state value of the consecutive state. This forms a cyclic constraint problem with as many
constraints and variables as there are system states. The solution is found through iteration and
due to the transitivity property of the state value update.
...
The advantage of reinforcement learning over dynamic programming is that (i) the problem space is
...
There are two main types of reinforcement learning algorithms known in literature. The commonly
applied discounted reinforcement learning method
...
evaluations, and average reward reinforcement learning
...
by splitting up the evaluation of each state into infinitely many parts. First, the average reward
per step, second, a bias value that specifies expected reward that is additionally accumulated when
traversing from the current state to the optimal path, and third, an infinite number of values of
which each includes the occurrence of the rewards and in which magnitude along the path
\citep{Howard64,Puterman94,
...
highest value of the parts, on ties selecting actions following the order above. That is, the
highest evaluation of the average reward, the bias value, the third component, the fourth component,
etc. The formulations for computing these values form nested constraints, so for instance the
current estimate of the average reward is used to compute the bias value. Therefore, a slight
approximation error may cause harmful damage in subsequent constraints, which makes average reward
reinforcement computationally very intensive. Worsening the issue, \citet[p.343ff]{Puterman94} shows
that an additional set of nested constraints have to be solved to gain the correct solution.


In contrast the standard discounted framework estimates only one single scalar value consisting of
the addition of these subterms, where however, the average reward is scaled by \(1/(1-\gamma)\).
%
To collect as much reward as possible, for instance to gain the highest expected profit, the
discount factor \(\gamma\) has to be very close to one, e.g. \(0.99\).
%
...
problematic when they are iteratively approximated. For this reasons most applications of discounted
reinforcement learning in literature evaluate their algorithms on processes that have an average
reward of about zero \citep{schneckenreither2020average}. \citet{schneckenreither2020average} shows
that tabular based discounted reinforcement learning agents are unable to find even basic policies
...
Therefore, we develop, adapt and enhance an algorithm that uses only one scalar value for the
estimation of the average reward over all states. Further it estimates for every state-action pair
the bias value and the sum of all other state value parts, usually referred to as error term. The
later values are adjusted by the average reward.
...
average reward adjusted reinforcement learning algorithm to dynamically set lead time for order
release. In contrast to the aforementioned works on average reward reinforcement learning our
...
\begin{itemize}
\item We propose a novel \textit{average reward adjusted deep reinforcement learning algorithm} \ARA{}.
  This is the first average reward adjusted reward learning algorithm that operates using a
  ...
  \citep[e.g.][]{mnih2015human}, which we advanced to prevent catastrophic forgetting by
...
reinforcement learning applications to lead time management, third we briefly review the relevant
notation to reinforcement learning and finally we present the relevant literature of reinforcement
learning in the rest of the section. The later includes a discussion of discounted reinforcement
learning, average reward reinforcement learning, the Laurent series expansion of the state values,
and finally average reward adjusted reinforcement learning.
...
and flow times \citealt{Pahl2007,hoyt1978dynamic}, but this may also substantially degrade the
...
\item \textsf{Reactive lead time management} sets lead times by reacting on earlier flow times
  (e.g., \citealt{enns2004work, Selcuk2006}), i.e.\@ data from the past.
...
it can be considered an exploratory research paper the main conclusions are that dynamically setting
...
the shop workload dependent \citep{Bertrand1983} due date assignment rules have been developed. The
...
assignment methods that incorporate neural networks exist \citet{Philipoom1994, Philipoom1997,
...
MIL extends the static BIL procedure to integrate shopfloor congestion by subtracting number of
queued jobs on the corresponding routing from the due date
\citep{RagatzMabert1988,wisner1995review}. For non-due-date oriented dispatching rules, their
...
using reinforcement learning. Their algorithm uses multiple agents to forecast the processing time
...
using reinforcement learning in a single product, four-station serial flow line. They compare its
...
tabular based reinforcement learning agent.
%
...
present improved results with asynchronous actor-critic reinforcement learning. Also games like
...
This section presents the average reward adjusted deep reinforcement learning algorithm \ARA{}.
...
of \(1\) in regard to the lead time of the current period. A naive approaches is to define one
action for each combination of possible lead time updates over all product types, e.g.\@ one action
for a system with six product types could be the vector \(+1, +1, 0, 0, +1, 0\). This however, due
to the exponential growth, results in only being able to solve very small production systems with
only few product types. A common solution in production planning is to aggregate the products into
product groups, cf.\@ hierarchical production planning~\citep{schneeweibeta1995hierarchical}.
%
...
product groups. In the sense of this foundational research paper, we rather aim for an enhanced
...
these extensions and with the objective to minimise the returned rewards. The idea is the same as in
Algorithm~\ref{alg:near}, except that the action selection procedure is a loop over all independent
agents and the implementation of the n-step logic.
%
