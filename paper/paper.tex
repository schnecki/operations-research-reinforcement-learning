...
optimising for the highest profit (or the lowest cost), the most intuitive and clear way of reward
feedback to the agent is presenting it the profit that is accumulated every period.


\MS{todo}
%
Therefore, we adapt an algorithm that uses a scalar value for the estimation of the average reward
over all states, and estimates for every state-action pair that incorporate the bias value and the
sum, usually referred to as error term, of all other parts. The later values are adjusted by the
average reward.
...
An extreme scenario of this problem is the so called ``lead time syndrome'', which describes a
vicious cycle where increasing flow times perpetually inflate the lead times which leads to worse
performance (see e.g., \citealt{Mather1978, knollmann2013control, Selcuk2006}).
%
