...
\begin{abstract}
  ...
  A major task in hierarchically structured manufacturing planning and control is to determine when
  to release orders to the shop floor. One key parameter is the lead time which is the planned time
  that elapses between the release of an order and its completion. Lead times are usually determined
  based on the flow times, i.e.\@ the measured duration orders take to traverse the production
  system. Traditional order release models assume static lead times, although it has been shown that
  they should be set dynamically to be able to react to the dynamic operational characteristics of
  production system. Therefore, we present a reinforcement learning order release model which sets
  lead times dynamically. The applied reinforcement learning agent is designed especially for
  processes with periodic feedback and high variability. This paper aims at providing a
  proof-of-concept for the proposed algorithm. We compare our method on a three-stage flow-shop
  simulation model not only to static order release techniques, but also to the commonly used deep
  Q-Learning agent. The results show that both reinforcement learning agents have room for
  improvement for queuing systems with low variability, that our proposed method usually outperforms
  deep Q-Learning and finally for systems with high utilisation that the proposed reinforcement
  learning algorithm performs very well, outperforming all other approaches.
  %
...
the lead times. Thus, a completely reactive approach can lead to an erratic order release behaviour,
the Lead Time Syndrome (see also \citealt{Mather1978, knollmann2013control, Selcuk2006}). We
hypothesis that dynamic lead time management approaches require a predictive design, i.e.\@ the
method has to incorporate a prediction of the future system state, to be able to compete with
state-of-the-art order release methods from literature. This is necessary in order to detect issues,
like the Lead Time Syndrome, in advance and react accordingly \citep{schneckenreither2020order}.
However, there only exist three papers that propose a predictive lead time management algorithm in
literature.

One method for which the future system state is by design considered a main driver of decision
making % that is designed to incorporate the future system state
is reinforcement learning \citep[e.g.][]{sutton1998introduction}, one of the main three
subcategories of machine learning. % It stems from dynamic programming the future
% system state is by design considered as a main driver of decision making in the current period.
Reinforcement learning (RL) is an optimisation technique that stems from dynamic programming,
therefore also seeks to compute the best stationary policy for a given problem. The idea is to
evaluate the current system state according to the direct feedback (reward), as well as the
previously learned state value of the consecutive state. This forms a cyclic constraint problem with
as many constraints and variables as there are system states. The solution is found through
iteration and due to the transitivity property of the state value update.
% as in dynamic programming is that the process and the current state and future states (or
% state-action pairs) of an underlying Markov Decision Process (MDP).
The advantage of RL over dynamic programming is that (i) the problem space is
explored by an agent and thus only expectantly interesting parts of the problem space need to be
assessed and (ii) the knowledge (acquisition) of transition probabilities becomes unnecessary as the
states are evaluated by consecutively observed states solely.

There exist two main types of deep RL algorithms in literature. The commonly
applied discounted RL method
\citep[e.g.][]{sutton1998introduction,mnih2015human,mnih2016asynchronous}, in which the future
system state is discounted by a factor \(0 < \gamma < 1\) to prevent an infinite growth of the state
evaluations, and average reward RL
\citep{Mahadevan96_AverageRewardReinforcementLearningFoundationsAlgorithmsAndEmpiricalResults,
  schwartz1993reinforcement,
  Mahadevan97_SelfimprovingFactorySimulationUsingContinuoustimeAveragerewardReinforcementLearning}.
%
% Like discounted reinforcement learning also average reward reinforcement learning is based on an
% oracle function, in our case the accumulated costs of a period, to assess the decisions taken by
% the agent. By repeatedly choosing different actions the agent examines the problem space and rates
% possible actions for any observed state. The advantage of average reward reinforcement learning
The advantage of the later % over the widely applied discounted reinforcement learning framework
is that the underlying optimisation technique is theoretically able to find better policies
% This yields from the fact that in the standard framework each state is assessed independently and
% by a single value. To be more precise, average reward reinforcement learning
by splitting up the evaluation of each state into infinitely many components. First, the average
reward per step, second, a bias value that specifies expected reward that is additionally
accumulated when traversing from the current state to the optimal path, and third, an infinite
number of values of which each includes the occurrence of the rewards and in which magnitude along
the path \citep{Howard64,Puterman94,
...
To expectantly accumulate the most reward one has to choose the action that leads to the state with
highest value of the components following the order above, on ties
selecting % within the best actions
according to the next component. That is, the highest evaluation of the average reward, the bias
value, the third component, the fourth component, etc. The formulations for computing these values
form nested constraints, so for instance the current estimate of the average reward is used to
compute the bias value. Therefore, a slight approximation error may cause harmful damage in
subsequent constraints, which makes average reward reinforcement computationally very intensive.
Worsening the issue, \citet[p.343ff]{Puterman94} shows that the constraint problem has to be
specified to solve one more component as originally intended to gain correct solution.


In contrast the standard discounted framework estimates only one single scalar value consisting of
the addition of these components, where however, the average reward is scaled by \(1/(1-\gamma)\).
%
But in order to collect as much reward as possible the discount factor \(\gamma\) has to be very
close to one, for instance \(0.99\).
%
% As the discount factor \(\gamma\) is usually set very
% close to \(1\), e.g. \(0.99\), which
Such a parameterisation leads to the fact that the subterm including the average reward dominates
the other two.
%
Further, in commonly applied unichain (and thus ergodic) MDPs the average reward per step is equal
over all states. Thus, independently assessing it is not only computationally unwisely, but also
problematic as they are iteratively approximated. For this reasons most applications of discounted
RL in literature evaluate their algorithms on processes that have an average
reward of about zero \citep{schneckenreither2020average}. \citet{schneckenreither2020average} shows
that tabular based discounted RL agents are unable to find even basic policies
when confronted with processes that have a small average reward per step. Unfortunately, when
optimising for the highest profit (or the lowest cost) in the area of operations management, the
most intuitive and clear way of reward feedback to the agent is presenting it the profit that is
accumulated every period.


Therefore, we develop an algorithm that uses only one scalar value for the estimation of the average
reward over all states. Further it estimates for every state-action pair the bias value and the sum
of all other state value components, usually referred to as error term. The later values are
...
%
%
Thus, as opposed to the commonly applied discounted RL algorithm we use an
average reward adjusted RL algorithm to dynamically set lead time for order
release. In contrast to the aforementioned works on average reward RL our
algorithm incorporates the optimisation of not only the average reward over time, but also the bias
values, which is an important adaption in highly stochastic systems. Only the work by
\citet{Mahadevan96_AnAveragerewardReinforcementLearningAlgorithmForComputingBiasoptimalPolicies}
integrates this second-level refinement optimisation. However, their average reward reinforcement
learning algorithm requires the selection of a reference state to prevent an infinite increase of
state values. This results from the lack of feedback in the iterative process of optimising
different sets of nested constraints. Furthermore, they asses the average reward independently for
each state.

To achieve a scalable lead time setting algorithm, we propose an extension that reduces the action
space tremendously. In fact, it transforms the problem size function measured on the number of
product types from one with an exponential increase to a linear one. The idea is that multiple
action decision agents are introduced that operate on the same state, but each one is responsible
for the lead time setting of only one product type.
% (i) To
...
Finally, we provide a proof-of-concept experimental section by testing the proposed algorithm on a
three stage flow shop with six product types, which provides ample evidence of the viability of the
approach.
% and are compared to static order release
% mechanisms. Furthermore, we show that the advantage of average reward reinforcement learning for the
% problem structures encountered in the context of production and logistics management by comparing
% the results to the discounted reinforcement learning
% framework.
The highly optimised value-iteration algorithm with improved n-step learning, small action-based
replay memories and concurrent workers infers order release policies which, especially in case of
high utilisation, outperform not only the static order release mechanisms, but also the commonly
applied deep Q-Learning algorithm~\citep[cf.][]{mnih2015human}.
% With regard to practical implications we are confident that decision support
% tools based on average reward reinforcement learning increase the decision quality of human
% planners.
%
In summary our contributions are as follows:
\begin{itemize}
\item We propose a novel \textit{average reward adjusted deep reinforcement learning algorithm}
  \ARA{}. This is the first average reward adjusted reward learning algorithm that operates using a
  artificial neural network as function approximation. It includes several known extensions like
  n-step learning \citep[e.g.][]{mnih2016asynchronous} or experience replay memory
  \citep[e.g.][]{mnih2015human}, which we aditionally advanced to prevent catastrophic forgetting by
  action-based selection of experiences.
\item We establish a action reduction method, named \textit{independent agents}, as a subset of the
  action space is treated as if there were multiple independent agents, although they share the same
  state. It reduces the growth of the number of possible actions from a exponential growth to a
  linear one. This is especially useful for product type based lead time management, as each agent
  is responsible for the lead time of one product type.
\item Using a three-stage flow shop model we show in an \textit{experimental evaluation} that the
  proposed algorithm outperforms standard Q-Learning and well established order release methods,
  especially for systems with high variability.
\end{itemize}
...
related literature. Section~\ref{sec:avg} establishes the average reward adjusted deep reinforcement
learning algorithm and the extension to independent agents.
Section~\ref{sec:Experimental_Evaluation} describes the simulation model we use to evaluate the
approach. In Section~\ref{sec:lead} we briefly discuss lead time setting in this paper, while the
results of the computation experiments are presented in Section~\ref{sec:Computational}. Finally
...
This section is split into several parts. First we discuss lead time management literature, then
RL applications to lead time management, third we briefly review the relevant
notation to RL and finally we present the relevant literature of reinforcement
learning in the rest of the section. The later includes a discussion of discounted reinforcement
learning, average reward RL, the Laurent series expansion of the state values,
and finally average reward adjusted RL.


\subsection{Related Lead Time Management Literature}

Setting lead times dynamically is a rather difficult task. Therefore, most state-of-the-art order
release mechanisms use static or fixed lead times to address the order release problem.
%
However, lead times should be set dynamically due to the non-linear relationship between utilisation
and flow times \citep{Pahl2007,hoyt1978dynamic}, but this may also substantially degrade the
system performance. For instance, a solely reactive approach can result in the lead time syndrome.
%
The \textit{lead time syndrome} describes a vicious cycle where increasing flow times perpetually
inflate the lead times which leads to worse performance (see e.g., \citealt{Mather1978,
  knollmann2013control, Selcuk2006}).
%
%
\citet{schneckenreither2020order} have established following categorisation of dynamic lead time
management approaches:

\begin{itemize}
\item \textsf{Reactive lead time management} sets lead times by reacting on earlier flow times
  (e.g., \citealt{enns2004work, Selcuk2006}), i.e.\@ data from the \textit{past}.
\item \textsf{Proactive lead time management} may incorporate \textit{past} data and the
  \textit{current} system state to set lead times (e.g., \citealt{Bertrand1983, ChungHuang2002}).
\item \textsf{Predictive lead time management} may incorporate \textit{past} data and the
  \textit{current} system state, and uses an anticipated \textit{future} system state to prevent
  arising issues of future periods (e.g.
  \citealt{PaterninaArboleda2001,schneckenreither2020order,Schneckenreither2019}).
\end{itemize}

% \MS[]{todo: Reactive}

In the sequel we review the lead time management (LTM) literature for due date assignment, order
release and scheduling, with the focus on order release literature.

\paragraph{Reactive LTM.}
%
%% Reactive Due Date Ass
Early due date assignment approaches \citep[e.g.][]{conway1967} set the due dates according to the
processing time (Total Work Content rule, TWK rule) or the number of operations.
%
%% Reactive Order Release
For order release \citet{enns2004work} investigate the use of a reactive approach with exponential
smoothing of flow times to set lead times for two-stage distribution requirements planning. Although
it is considered an exploratory research paper the main conclusions are that dynamically setting
planned lead times using exponential smoothing works well, but that the reactions are delayed. The
later is due to the nature of the flow time measurement only being possible for finished orders.
%
Another reactive approach uses exponential smoothing to forecast flow times to set lead times for a
capacitated multi-stage make-to-stock system \citep{Selcuk2006}. Their approach performs better than
fixed lead time techniques for most experiments. However, they also exhibit the LTS\@.

\paragraph{Proactive LTM.}
%
%% Proactive Due Date Assignment
The TWK due date assignment rules have been extended by several researchers to proactive methods
\citep{eilon1976, kanet1986, Enns1995}. As flow times depend on the workload and machine capacity in
the shop \citep{Bertrand1983} workload dependent due date assignment rules have been developed. The
basic idea is to estimate the flow time of an order on its arrival by measuring the number of orders
queued on the corresponding routing, which can be analytically \citep[e.g.][]{ChungHuang2002}, by
the use of a simulation model \citep[e.g.][]{vig1991}, statistical methods \citep[e.g.][]{Tai2012}
or using hybrid approaches \citep[e.g.][]{Kaplan1993}. Furthermore several proactive due date
assignment methods that incorporate neural networks exist \citep{Philipoom1994, Philipoom1997,
  HsuSha2004,Patil2008,ChangetalFuzzyDDset2008}.
%
%% Proactive Order Release
In the case of order release the analytical papers by \citet{Selcuk2009} and \citet{Selcuk2013} use
the WIP level to quote lead times. In the former they show that the less frequently updating the
lead time decreases the nervousness and improves the performance, while in the latter that
dynamically updating lead times performs much better than static lead times.
%
% MIL (Modified infinite loading)
Modified infinite loading extends the static Backward Infinte Loading (BIL) procedure to integrate
shopfloor congestion by subtracting number of queued jobs on the corresponding routing from the due
date \citep{RagatzMabert1988,wisner1995review}. For non-due-date oriented dispatching rules, their
approach has a positive effect on total cost performance, while for due-date oriented dispatching
this holds only if the due dates are loose. For all other cases the release algorithm has no
negative effect on performance. Variants of this rule-based method exist
\citep[e.g.][]{park1989job,kim1995evaluating}.
% Bertrand 2002
% \citet{bertrand2002investigation} investigate different ways of releasing orders, assigning lead
% times and operation due dates to assembly orders. In their work a due date is set once the order
% arrives and then a release time calculated by subtracting the lead time from it. In one approach the
% lead time calculation is the sum of the operation times plus a factor consisting of the waiting time
% allowance per operation times the number of operations. They use the average operation waiting time
% as waiting time allowance factor. In the other approaches the orders are released using the
% \ac{imre} policy. In terms of order release they find that all work orders of an assembly order
% should be released at the same time.
%
A machine learning approach is presented by \citet{Lee1997}. They combine a genetic algorithm with a
decision tree to generate sequencing rules in the order pool for release. In the tested job shops
the CONWIP rule determines the release of the orders and a decision tree to find the order release
sequence. Their method leads to significant improvements over conventional single release and
dispatching approaches.


%% Predictive
\paragraph{Predictive LTM.}
%
Due to the complex problem structure we hypothesise that dynamic lead time management approaches
should aim for a predictive design in order to be able to compete with state-of-the-art order
release methods from literature. However, there only exist four papers that propose a predictive
lead time management method.

For scheduling \citet{wang2007reinforcement} present a predictive lead time management approach
using RL. Their algorithm uses multiple agents to forecast the processing time
of each job for routing decisions. They show on a 10 machine job shop that the RL algorithm performs
equally or better than tested heuristics.
%
In case of order release, \citet{PaterninaArboleda2001} introduce a predictive order release model
using RL in a single product, four-station serial flow line. They compare its
performance (WIP costs) with conventional order release policies (e.g., Kanban and CONWIP). The
algorithm decides on whether or not to release an order after each system change, the completion of
an operation of any order at any stage or a new order arrival, and assumes that any unsatisfied
demand is lost. Thus, they use a continuous order release method although in practice order release
decisions often need to be made on a periodical basis, e.g.\@ daily (see
\citealt{enns2004work,GeldersvanW1982}). They outperform existing control policies with their
tabular based RL agent.
%
Then, \citet{Schneckenreither2019} use several different RL algorithms to make
...
prevent the negative effects of the syndrome. Their model outperforms static and reactive lead time
management approaches, especially for scenarios with high utilisation.


\subsection{Related Reinforcement Learning Applications}

Over the past decades RL has been applied to various problems, for which
astonishing results have been reported. E.g.\@ only recently \citet{mnih2015human} have presented a
novel value-iteration RL agent which exceeds human-level abilities in playing
many classic Atari games~\citep{bellemare2012investigating}. Further, \citet{mnih2016asynchronous}
present improved results with asynchronous actor-critic RL. Also games like
Go~\citep{silver2016mastering} and Chess~\citep{silver2017mastering} have been mastered with
superhuman performance by \textit{tabula rasa} RL agents. Furthermore, the
...
\citep{zhang1995reinforcement,wang2005application} or scheduling \citep{zhang1995reinforcement,
  waschneck2018optimization}.
%
However, all these applications use discounted RL and are either designed to investigate a rather
simple MDP, e.g.\@ by selecting heuristics instead of optimising the underlying problem itself, or
by mapping the actual objective in a reward function that is approximately zero on average over
time.
%
This is due to the fact that state value is largely composed of a term defined by the current
policys' average reward value \citep{MillerVeinott1969,Blackwell62} which would otherwise dilute the
state values and thus decrease the solution quality, as can for instance be observed in
...
function use average reward RL. \citet{aydin2000dynamic} use it in the setting
...
After these foundational works for average reward RL they introduced a
continuous time average reward RL algorithm named SMART
...
% integrates this second-level refinement optimisation. However, their average reward reinforcement
% learning algorithm requires the selection of a reference state to prevent an infinite increase of
% state values. This results from the lack of feedback in the iterative process of optimising the
% different refinement optimisation levels. Furthermore, they asses the average reward independently
% for each state.


\subsection{Reinforcement Learning Preliminaries}

The rest of this section briefly reintroduces the important concepts of average reward adjusted
RL, elaborates on optimality criteria and provides insights of the underlying
algorithm. For a more extensive introduction to average reward RL we refer to
\citet{schneckenreither2020average}.

Like \citet{MillerVeinott1969} we are considering problems that are observed in a sequence of points
...
Markov property \citep[p.63]{sutton1998introduction}. RL processes that possess
...
printer-loop is preferred over the mail-loop, although the mail-loop accumulates more reward over
time, i.e.\@ selecting the mail-loop is a sub-optimal choice. Observe that the average reward
received per step equals \(1\) for the printer-loop and \(2\) for the mail-loop. Thus, the
(Blackwell-)optimal policy is to choose the mail-loop. However, if
\(\gamma < 3^{-\frac{1}{5}} \approx 0.8027\) an agent using discounted RL
chooses the printer loop as doing the printer loop has a higher evaluation.
%
\begin{figure}[t!]
  \centering
...
% learning with average reward RL, but also connects to \(n\)-discount-optimality.
%
...
difficult. Average reward RL overcomes these issues by separately assessing the
...
% a divide and conquer methodological approach to RL. Thus in average reward
% RL these terms are learned separately, while in the discounted framework a
...
\(W^{\pol}_{M}\) is offset by the vector \(u\), where for the transition probability matrix \(P\)
the vector \(u\) is characterised by \((I-P)u = 0\). Note that \(u\) is determined by the number of
...
s_{t} = s]\). The corresponding Bellman optimality equation then is
\(\X_{\gamma}^{\polopt}(s) = \max_{a} \E_{\polopt}[r_{t} + \gamma \X_{\gamma}^{\polopt}(s_{t+1}) -
\avgrew^{\pol} \mid s_{t} = s]\). Turning this formula into an update rule it builds the foundation
of our algorithm.


\section{Average Reward Adjusted Deep RL Algorithm }\label{sec:avg}


This section presents the average reward adjusted deep RL algorithm (\ARA{}).
First, we present a basic version, which suffices for small problem sizes and introduce the idea of
independent agents, before presenting an scalable optimised version.

\subsection{Basic Average Reward Adjusted Deep RL Algorithm}
...
on a target network \(\netTarget{}\) and a worker network \(\netWorker{}\) as in deep Q-Learning
\citep{mnih2015human}. % The target network reduces fluctuations in the state value estimates and
...
observed rewards. This makes sense as the later rather describes policies from the past.
...
In our application the agent can update the lead time for each product type by a maximum magnitude
of \(1\) in regard to the lead time of the current period. A naive approach is to define one action
for each combination of possible lead time updates over all product types, e.g.\@ one action for a
system with six product types could be the vector \((+1, +1, +0, +0, +1, +0)\) for which each
component defines the update of the corresponding product type lead time from the current lead time.
...
products as done by \citet{schneckenreither2020order}, we refrain from this adaption as it does not
scale to larger sized production systems.
...
during the learning process as the agent starts to behave differently.
%


\subsection{Scalable Average Reward Adjusted Deep RL Algorithm}

The basic algorithm above suffices only for problem sets of rather small size due to the
computational complexity and the growth of the action space. In our case, that would either require
a production system with at most \(3\) product types, or an aggregation of product types into
product groups. In the sense of this foundational research paper, we aim for an enhanced
version of the algorithm that is scalable.
...
Therefore, our implementation includes several optimisations, that help the algorithm in better
exploring the solution space and stabilise the learning process. These include the above discussed
independent agents that operate on the same state, \(n\)-step learning \citep{mnih2016asynchronous},
and ensuring to constantly reduce the average reward as searching for policies with higher average
costs is unnecessary. Algorithm~\ref{alg:full} depicts a detailed view on the algorithm including
these extensions and with the objective to minimise the returned rewards, as we aim for minimal
costs in our experiments. The idea is the same as in Algorithm~\ref{alg:near}, except that the
action selection procedure is a loop over all independent agents and the implementation of the
n-step logic.
%
The agent first chooses an action (steps \(4-5\)), executes the selected action and stores the
experience in the replay memory (step \(6\)) and then samples a mini-batch of experiences each of
length \(nstep\) to improve the policy (steps \(7-21\)). Step \(17\) uses an exponentially smoothed
overestimation (by \(2.5\%\)) of the average reward and rate \(2^{-5}\). All average reward updates
above this overestimation are rejected.
...
%%%%%%
% Calculation
% (285 / (3 * u)) = interarrival time
%
% 285... Proc time bottleneck
% 3  ... every third product routes of this machine
% u  ... utilisation, e.g. 0.90
...
\section{Lead Time Setting}\label{sec:lead}

In this section we define the tested lead time setting algorithms. This includes a description of
the MDP used for the reinforcement learning agents.
...
\subsection{Average Reward Adjusted Deep RL Algorithm}
...
currently set lead times $\lt_{\pt} \in \{1,2,\ldots,\DDS\}$ for each product type $\pt$.
%
We restrict the lead time update for state $s_{t+1}$ according to the lead time $\lt_{\pt}$ from state
$s_{t}$ by a maximum change of 1. Thus, if $\lt_{\pt}$ is the current lead time of product type $\pt$
the agent can choose a lead time in $\{1,2,\ldots,\DDS\} \cap \{\lt_{\pt}-1,\lt_{\pt},\lt_{\pt}+1\}$. % Put
...
% type.
%
Recall that we implement independent agents to reduce the action space. In particular, this adaption
enables us to present the algorithm the above defined state space not only on the basis of product
groups, but directly for each product type. Note that without this extension the problem is
computationally infeasible due to \(3^{6} = 729\) possible actions. Removing the dependency between
the lead times of different product types reduces the set of possible actions, and thus the number
of ANN output layer nodes, to \(3 \cdot 6 = 18\). In preliminary experiments on small examples we
only observed a very slight increase of required steps during training. Therefore, this is a vital
adaption to ensure scalability.
...
maximum \(5\), the lead times with minimum \(0\) and maximum \(7\).
...
sum of gradients backpropagated trough the worker network using Adam \citep{kingma2014adam}, where
...
\(\BILTwo\) performs best in the sense of accumulated costs (\(220.53\)). The RL
...
RL agents have a higher work in process, which is clear as they try to smooth
...
there is still potential for the improvement of RL variants.
...
% RL algorithms, which lead to the development of
...
This paper is one of the first papers that use RL for setting lead times to
...
RL algorithm. We hypothesise that due to the more correct state-action value
...
% RL. The performance is tested on a multi-product, two-stage hypothetical flow
...
% parameterisation and resolving current issues when scaling average RL to complex
% scenarios.

% \MS[t]{todo}

\bibliographystyle{tfcadinitials}
...
\begin{algorithm}[h!]
  \begin{algorithmic}[1]
    ...
    \begin{minipage}[t]{\dimexpr\textwidth-\leftmargin-\labelsep-\labelwidth-\leftmargin+2pt}
      With probability \(\frac{\pexp}{n}\) choose action \(a_{t,i}\) randomly or otherwise the one
      fulfilling
