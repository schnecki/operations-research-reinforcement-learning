...
average reward \(\avgrew^{\pol}\). Algorithm~\ref{alg:full} depicts the algorithm including the
extensions.
...
\begin{algorithm}[t!]
  \begin{algorithmic}[1]
    ...
    \hspace{2.4cm}\(\avgrew^{\pol} \gets (1- \alpha) \avgrew^{\pol} + \alpha [r_{t} +
      ...
      \netWorker{})]\)
    \EndIf
    \State{Compute overestimate of \(\avgrew^{\pol}\) and store in \(\avgrew_{\Overestim}^{\pol}\)}
