...
In the sequel we briefly present the needed formalism of average reward adjusted RL, where for a
more comprehensive version see \citep{schneckenreither2020average}. \MS[t]{todo}


\begin{definition}
  Due to \citet{schneckenreither2020average} the \textit{average reward adjusted (discounted) state
    value} $\X_{\gamma}^{\pol}(s)$ of a state $s$ under policy $\pol$ and with discount factor
  $0 \leqslant \gamma \leqslant 1$ is given as
  \(\X_{\gamma}^{\pol}(s) \defsym V^{\pol}(s) + e_{\gamma}^{\pol}(s)\).
\end{definition}
%
With the Laurent Series expensive this reformulates to
...
\(\X_{\gamma}^{\pol}(s) = \E_{\pol}[r_{t} + \gamma \X_{\gamma}^{\pol}(s_{t+1}) - \avgrew^{\pol}
\mid s_{t} = s]\). By turning this formula into an update rule it builds the foundation of our
algorithm.
...
\citet{schneckenreither2020average} and depicted in Algorithm~\ref{alg:near}.
%
%
%
