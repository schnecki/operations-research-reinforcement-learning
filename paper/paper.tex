  ...
  \begin{algorithmic}[1]
    ...
    \begin{align*}
      ...
      \max_{a}\X^\pol_{\gamma_1}(s_{t+1},a; \netWorker{}) - \X^\pol_{\gamma_1}(s_{t},a_{t};
                        \netWorker{})]
      % \\ \avgrew^{\pol}  & \gets \max(\avgrew^{\pol}, \avgrew_{\min}^{\pol})\\
      % \avgrew_{\min}^{\pol} & \gets (1 - \alpha_{\min}) \avgrew_{\min}^{\pol} + \alpha_{\min} 0.975
      % \cdot \avgrew^{\pol}
    \end{align*}
    \EndIf
...
\begin{algorithm}[t!]
  \begin{algorithmic}[1]
    ...
    \end{minipage}
    \For{each mini-batch}
    \For{each experience, starting with the latest seen experience \(\nstep\)}
    ...
    \hspace{2.4cm}\(\avgrew^{\pol} \gets (1- \alpha) \avgrew^{\pol} + \alpha [r_{t} +
      ...
      \netWorker{})]\)
    \EndIf
    \State{\(\target_{\gamma_0}  \gets r_{t} + \gamma_{0} \target_{\gamma_{0}} - \avgrew^{\pol}  \)}
    \State{\(\target_{\gamma_{1}}  \gets r_{t} + \gamma_{1} \target_{\gamma_{1}} - \avgrew^{\pol}\)}
